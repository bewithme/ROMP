...
def romp_settings(input_args=sys.argv[1:]):
    ...
    parser.add_argument('-o', '--save_path', type=str, default=osp.join(osp.expanduser("~"), 'ROMP_results'),
    ...
    parser.add_argument('--smpl_path', type=str, default=osp.join(osp.expanduser("~"), '.romp', 'smpl_packed_info.pth'),
                        help='The path of smpl model file')
    parser.add_argument('--model_path', type=str, default=osp.join(osp.expanduser("~"), '.romp', 'ROMP.pkl'),
                        help='The path of ROMP checkpoint')
    parser.add_argument('--model_onnx_path', type=str, default=osp.join(osp.expanduser("~"), '.romp', 'ROMP.onnx'),
...
class ROMP(nn.Module):
    def __init__(self, settings):
        ...
        self.center_map_parser = CenterMap(conf_thresh=self.settings.center_thresh)
        self.device = determine_device(self.settings.GPU)
        self.build_model()
        self.initilization()
    ...
    def build_model(self):
        if not self.settings.onnx:
            model = ROMPv1().eval()
            model.load_state_dict(torch.load(self.settings.model_path, map_location=self.device))
    ...
    def initilization(self):
        ...
        if self.settings.calc_smpl:
            self.smpl_parser = SMPL_parser(self.settings.smpl_path).to(self.device)
    ...
    def single_image_forward(self, image):
        ...
        if self.settings.onnx:
            ...
            center_maps, params_maps = torch.from_numpy(center_maps).to(self.device), torch.from_numpy(params_maps).to(
                self.device)
        else:
            center_maps, params_maps = self.model(input_image.to(self.device))
        params_maps[:, 0] = torch.pow(1.1, params_maps[:, 0])
        parsed_results = parsing_outputs(center_maps, params_maps, self.center_map_parser)
    ...
    def temporal_optimization(self, outputs, signal_ID):
        ...
        if self.settings.show_largest:
            ...
            outputs['smpl_thetas'], outputs['smpl_betas'], outputs['cam'] = outputs['smpl_thetas'].unsqueeze(0), \
                                                                            outputs['smpl_betas'].unsqueeze(0), outputs[
                                                                                'cam'].unsqueeze(0)
        else:
