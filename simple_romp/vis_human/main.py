...
import torch
import numpy as np
from vis_human.vis_utils import mesh_color_left2right, mesh_color_trackID, rotate_view_perspective, \
    rendering_mesh_rotating_view, \
...
def rendering_romp_bev_results(renderer, outputs, image, rendering_cfgs, alpha=1):
    ...
    if rendering_cfgs['renderer'] == 'sim3dr':
        ...
        if 'mesh_bird_view' in rendering_cfgs['items']:
            verts_bird_view, bbox3D_center, scale = rotate_view_weak_perspective(verts_tran, rx=-90, ry=0,
                                                                                 img_shape=background.shape[:2],
                                                                                 expand_ratio=1.2)
        ...
        if 'mesh_side_view' in rendering_cfgs['items']:
            verts_side_view, bbox3D_center, scale = rotate_view_weak_perspective(verts_tran, rx=0, ry=-90,
    ...
    if rendering_cfgs['renderer'] == 'pyrender':
        ...
        if 'mesh_bird_view' in rendering_cfgs['items']:
            ...
            rendered_bv_image, rend_depth = renderer(verts_bird_view.cpu().numpy(), triangles, background, persp=False,
        ...
        if 'mesh_side_view' in rendering_cfgs['items']:
            ...
            rendered_sv_image, rend_depth = renderer(verts_side_view.cpu().numpy(), triangles, background,
                                                     mesh_colors=mesh_colors)
        ...
        if 'rotate_mesh' in rendering_cfgs['items']:
            ...
            frame_rate = 24
            writer = cv2.VideoWriter(save_path, cv2.VideoWriter_fourcc(*'mp4v'), frame_rate,
                                     rotate_renderings[0].shape[:2])
    ...
    if 'pj2d' in rendering_cfgs['items']:
        img_skeleton2d = draw_skeleton_multiperson(copy.deepcopy(image), outputs['pj2d_org'].cpu().numpy()[:, :24],
                                                   mesh_colors * 255)
    ...
    if 'center_conf' in rendering_cfgs['items']:
        for ind, kp in enumerate(outputs['pj2d_org'].cpu().numpy()[:, 0]):
            cv2.putText(result_image[1], '{:.3f}'.format(outputs['center_confs'][ind]), tuple(kp.astype(int)),
                        cv2.FONT_HERSHEY_COMPLEX, 1, (255, 0, 255), 1)

    if 'tracking' in rendering_cfgs['items'] and 'track_ids' in outputs:
        for ind, kp in enumerate(outputs['pj2d_org'].cpu().numpy()[:, 0]):
            cv2.putText(result_image[1], '{:d}'.format(outputs['track_ids'][ind]), tuple(kp.astype(int)),
